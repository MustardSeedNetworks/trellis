...
| R5 | **cgo creep** erodes the clean Go core | Med | Engine is a *separate process*, not linked. cgo confined to capture daemon; prefer pure-Go (`modernc.org/sqlite`). Enforce via build/CI check. |
