...
- **Predictive RF engine** (C++/GPU): walls, materials, antennas, propagation models,
  AP placement, tx-power → _predicted_ heatmaps; calibration (fit predicted→measured).
...
**Go core**, lifted from Seed. That **narrows the C++/GPU engine to _prediction only_**
...
4. **Add** the predictive engine alongside; reuse the _same_ heatmap/colorscale render.
